...
function TokenAuthenticationScheme(server, options) {
  ...
  this._initializeCookie = function() {
    ...
    var cookieOptions = {
      ...
      isSecure: options.cookie.isSecure === undefined
          ? cookieDefaults.isSecure
          : options.cookie.isSecure,
      isHttpOnly: options.cookie.isHttpOnly === undefined
          ? cookieDefaults.isHttpOnly
          : options.cookie.isHttpOnly,
  ...
  this._extractTokenFromCookie = function(request) {
    var cookieName = options.cookie.name || TokenAuthenticationScheme.DEFAULT_AUTH_COOKIE_NAME;
    var authCookie = request.state[cookieName];
    if (!authCookie) {
      return null;
    }
